...
#         result = ezbox.add_arp_entry(temp_ip_address, "a:1:2:3:4:5")    
    ...
    unused_ip_address = ezbox.get_unused_ip_on_subnet("255.255.248.0")
    result = ezbox.add_arp_entry(unused_ip_address[0], "a:1:2:3:4:5")
    if result == False:
    ...
    print unused_ip_address[0]
    
    result = ezbox.check_cp_entry_exist(unused_ip_address[0], "a:1:2:3:4:5", True)
...
if 2 in scenarios:
    ...
    print unused_ip_address[1]
    
    result = ezbox.add_arp_entry(unused_ip_address[1], "a:1:2:3:4:5")
    if result == False:
        print "Error - add entry finished with error exit code"
        exit(1)
    key = hex(ip2int(unused_ip_address[1]))
    key = key[2:]
    key = '0'*(8-len(key)) + key
    key = list(key)
    key = ' '.join(key)
    ...
    result = ezbox.check_cp_entry_exist(unused_ip_address[1], "a:1:2:3:4:5", update_arp_entries=True)
    if result == True:
    ...
    ezbox.delete_arp_entry(unused_ip_address[1])
