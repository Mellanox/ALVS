...
def user_init(setup_num):
	...
	for i in range(server_count):
		server_list.append(HttpServer(ip = setup_list[index]['ip'],
						  ...
						  username = "root", 
						  password = "3tango", 
						  vip = vip_list[i/2],#servers0,1->service 0 ..... servers8,9->service 4
						  eth='ens6'))
		index+=1
...
def run_user_test(server_list, ezbox, client_list, vip_list):
	...
	for i in range(server_count):
		server_list[i].update_vip(vip_list[((i/2) + 1)%service_count])#servers0,1->service 1 ..... servers8,9->service 0
	for server in server_list:
		ezbox.add_server(server.vip, port, server.ip, port)
	for index, client in enumerate(client_list):
