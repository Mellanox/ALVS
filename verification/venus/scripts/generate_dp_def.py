...
output_dir = alvs_root + "/verification/venus/output/dp/" + sys.argv[3]
...
cmd = "gcc -fno-inline -D__inline= -Dalways_inline= -D__always_inline__=__noinline__ -nostdinc -nostartfiles -nodefaultlibs -nostdlib -fdump-rtl-expand -fpack-struct -gdwarf-2 -g3 -ggdb -w -Dasm= '-Dvolatile(...)=' -Dmain=my_main %s -DNPS_BIG_ENDIAN -O0 -c -isystem %s/src/common/ -I%s/EZdk/ldk/toolchain/lib/gcc/arceb-ezchip-linux-uclibc/4.8.4/include-fixed/ -I%s/EZdk/ldk/toolchain/lib/gcc/arceb-ezchip-linux-uclibc/4.8.4/include -I%s/EZdk/ldk/toolchain/arceb-ezchip-linux-uclibc/sysroot/usr/include/ -I%s/src/common -I%s/src/dp -I/mswg/release/nps/EZdk/EZdk-17.0300/dpe/dp/include -I/mswg/release/nps/EZdk/EZdk-17.0300/dpe/frame/include -I/mswg/release/nps/EZdk/EZdk-17.0300/dpe/sft/include -I/mswg/release/nps/EZdk/EZdk-17.0300/dpe/dpi/include -Wl,--warn-unresolved-symbols -c " %(app_flag, alvs_root, alvs_root, alvs_root, alvs_root, alvs_root, alvs_root)
for module_name in sys.argv[3:]:
	cmd += "[[SRC:DUT_%02d]] " %i
	i += 1	
