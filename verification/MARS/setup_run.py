...
def main():
	...
	if options.setup_num:
		for line in fd_of_all_setups:
			if line[0] != '#' and line.strip() == options.setup_num:
				...
				if is_locked == False:		#someone another use this setup
					fd_of_curr_setup = open_file(file_path_of_curr_setup, 'r')
					curr_setup_inf = fd_of_curr_setup.read()
					print  "setup " + options.setup_num + " is locked. used by " + curr_setup_inf.split('\t')[0]\
	...
	try:	# Ctrl+C options
		...
		if options.run_regression.lower() == "true":
			...
			setup_num = line[:-1]
			os.system(currentdir + '/MARS_regression_run.py -s '+ setup_num + path + tags + file_name)
		#####   hold setup
		if options.hold.lower() == "true":
